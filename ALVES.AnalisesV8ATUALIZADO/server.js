const express = require("express"),
  ...
  path = require("path"),
  fs = require("fs"),
  SQLiteStore = require("connect-sqlite3")(session); // <-- Adicionado o armazenador SQLite
...
app.use(session({
  ...
  cookie: {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production", // Ativa segurança HTTPS automaticamente na Render
...
app.post("/api/register", (req, res) => {
  ...
  try {
    db.prepare("INSERT INTO users(name,email,password_hash) VALUES(?,?,?)").run(name.trim(), email.trim().toLowerCase(), bcrypt.hashSync(password, 12));
...
app.post("/api/login", (req, res) => {
  let u = db.prepare("SELECT * FROM users WHERE email=?").get(String(req.body.email || "").trim().toLowerCase());
  ...
  if (u.status !== "approved") return res.status(403).json({ error: u.status === "pending" ? "Cadastro aguardando aprovação." : "Acesso blocoado." });
...
app.get("/api/leagues", auth, (req, res) => res.json(db.prepare("SELECT id,country,name,csv_name,games_count,updated_at FROM leagues ORDER BY country,name").all()));

app.get("/api/leagues/:id", auth, (req, res) => {
  const l = db.prepare("SELECT * FROM leagues WHERE id=?").get(req.params.id);
  l ? res.json(l) : res.status(404).json({ error: "Liga não encontrada" })
});

const upload = multer({ storage: multer.memoryStorage(), limits: { fileSize: 30 * 1024 * 1024 } });

app.post("/api/leagues/:id/csv", admin, upload.single("file"), (req, res) => {
  const l = db.prepare("SELECT * FROM leagues WHERE id=?").get(req.params.id);
  if (!l) return res.status(404).json({ error: "Liga não encontrada" });
  if (!req.file) return res.status(400).json({ error: "Envie um CSV." });
  const data = req.file.buffer.toString("utf8"), games = Math.max(0, data.replace(/\r/g, "").split("\n").filter(x => x.trim()).length - 1);
  db.prepare("UPDATE leagues SET csv_name=?,csv_data=?,games_count=?,updated_at=CURRENT_TIMESTAMP WHERE id=?").run(req.file.originalname, data, games, l.id);
...
app.patch("/api/users/:id", admin, (req, res) => {
  ...
  if (id === req.session.user.id && req.body.status === "blocked") return res.status(400).json({ error: "Não bloqueie sua própria conta." });
  db.prepare("UPDATE users SET status=COALESCE(?,status),role=COALESCE(?,role) WHERE id=?").run(req.body.status || null, req.body.role || null, id);
  res.json({ ok: true })
});

app.delete("/api/users/:id", admin, (req, res) => {
  const id = +req.params.id;
  if (id === req.session.user.id) return res.status(400).json({ error: "Não exclua sua própria conta." });
